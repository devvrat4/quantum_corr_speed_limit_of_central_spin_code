...
def rho(t):
    ...
    rhot[0, 0] = rho0[0, 0]*(1 - alpha(t)) + rho0[1, 1]*beta(t)
    rhot[1, 1] = 1 - rhot[0, 0]
    rhot[0, 1] = rho0[0, 1]*Delta(t)
...
def kraus(t):
    ...
    X1 = (1 - (alpha(t)+beta(t))/2) + (1/2)*math.sqrt((alpha(t) - beta(t))**2 + 4*abs(Delta(t))**2)
    X2 = (1 - (alpha(t)+beta(t))/2) - (1/2)*math.sqrt((alpha(t) - beta(t))**2 + 4*abs(Delta(t))**2)
    Y1 = (math.sqrt((alpha(t) - beta(t))**2 + 4*abs(Delta(t))**2) - (alpha(t) - beta(t)))/(2*abs(Delta(t)))
    Y2 = (math.sqrt((alpha(t) - beta(t))**2 + 4*abs(Delta(t))**2) + (alpha(t) - beta(t)))/(2*abs(Delta(t)))
    
    theta = np.arctan(Delta(t).imag/Delta(t).real)
    
    K1[0, 1] = math.sqrt(beta(t))
    K2[1, 0] = math.sqrt(alpha(t))
