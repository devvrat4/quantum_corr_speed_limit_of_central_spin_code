...
def a1(t):
    ...
    for m in range(M+1):
        factm = -(w/T)*(m*(1 - (m-1)/(2*M)) - 1/2)
        modA1m = modA1m + np.exp(factm)*(abs(A1(m, t)))**2
    modA1m = (1/Z)*modA1m
    return modA1m
...
def b1(t):
    ...
    for m in range(M+1):
        factm = -(w/T)*(m*(1 - (m-1)/(2*M)) - 1/2)
        modB1m = modB1m + (m+1)*np.exp(factm)*(abs(B1(m, t)))**2
    modB1m = (1/Z)*modB1m
    return modB1m
...
def c1(t):
    ...
    for m in range(M+1):
        factm = -(w/T)*(m*(1 - (m-1)/(2*M)) - 1/2)
        modC1m = modC1m + np.exp(factm)*(abs(C1(m, t)))**2
    modC1m = (1/Z)*modC1m
    return modC1m
...
def g1(t):
    ...
    for m in range(M+1):
        factm = -(w/T)*(m*(1 - (m-1)/(2*M)) - 1/2)
        A1C1s = A1C1s + A1(m, t)*np.conjugate(C1(m, t))*np.exp(factm)
    A1C1s = (1/Z)*A1C1s
    return A1C1s
...
def d1(t):
    ...
    for m in range(M + 1):
        factm = -(w/T)*(m*(1 - (m-1)/(2*M)) - 1/2)
        modD1m = modD1m + m*np.exp(factm)*(abs(D1(m, t)))**2
    modD1m = (1/Z)*modD1m
    return modD1m
...
def kraus(t):
    ...
    R1 = (1/2)*(a1(t) + c1(t) + np.sqrt((a1(t) - c1(t))**2 + 4*(abs(g1(t)))**2))
    R2 = (1/2)*(a1(t) + c1(t) - np.sqrt((a1(t) - c1(t))**2 + 4*(abs(g1(t)))**2))
    T1 = (0.5/abs(g1(t)))*(a1(t) - c1(t) + np.sqrt((a1(t) - c1(t))**2 + 4*(abs(g1(t)))**2))
    T2 = (0.5/abs(g1(t)))*(a1(t) - c1(t) - np.sqrt((a1(t) - c1(t))**2 + 4*(abs(g1(t)))**2))

    theta = np.arctan((g1(t).imag)/(g1(t).real))
...
np.savetxt(f"data_qsl_{M}_tau_int_on", data)
