...
# plt.plot(t, rho_11, label = r"$\rho_{11}$")
# plt.plot(t, rho_22, label = r"$\rho_{22}$")
# plt.plot(t, rho_33, label = r"$\rho_{33}$")
...
y1 = np.zeros((len(t), dim*dim), dtype=complex)
...
for i in range(len(t)):
    ...
    tr_dist_rho = rho - rho0
    
    tr_dist_eig = eigvals(tr_dist_rho)
    
    for u in range(len(tr_dist_eig)):
        tr_dist[i] = tr_dist[i] + abs(tr_dist_eig[u])
    tr_dist[i] = (1/2)*tr_dist[i]
